...
DOWNLOAD_ROOT = "http://data.insideairbnb.com/germany/be/berlin/2020-03-17/"
DATA_PATH = os.path.join("datasets", "AirBnB")
DATA_URL = DOWNLOAD_ROOT + "visualisations/listings.csv"

def fetch_AirBnB_data(data_url=DATA_URL, data_path=DATA_PATH):
...
#%%
# 1.1 Alternative 1: generate id with static data
AirBnB_with_id["id"] = AirBnB["longitude"] * 1000 + AirBnB["latitude"]
train_set, test_set = split_train_test_by_id(AirBnB_with_id, 0.2, "id")
...
#     "Overall": price_cat_proportions(AirBnB),
...
AirBnB.plot(kind="scatter", x="longitude", y="latitude", title="bad_visualization_plot")
...
#%%
AirBnB.plot(kind="scatter", x="longitude", y="latitude", alpha=0.2,
    s=AirBnB["number_of_reviews"], label="number_of_reviews", figsize=(10,7),
    c="price", cmap=plt.get_cmap("jet"), colorbar=True,
    sharex=False, title="AirBnB_prices_scatterplot")
plt.legend()
save_fig("AirBnB_prices_scatterplot")

#%%
# AirBnB.plot(kind="scatter", x="price_cat", y="median_house_value",
#              alpha=0.1, title="income_vs_house_value_scatterplot")
# plt.axis([0, 16, 0, 550000])
# save_fig("income_vs_house_value_scatterplot")

# # create new variables
# AirBnB["rooms_per_household"] = AirBnB["total_rooms"]/AirBnB["households"]
# AirBnB["bedrooms_per_room"] = AirBnB["total_bedrooms"]/AirBnB["total_rooms"]
# AirBnB["population_per_household"]=AirBnB["population"]/AirBnB["households"]
# corr_matrix = AirBnB.corr()
# corr_matrix["median_house_value"].sort_values(ascending=False)
# AirBnB.plot(kind="scatter", x="rooms_per_household", y="median_house_value",
...
AirBnB = train_set.drop("price", axis=1) # drop labels for training set
...
print(AirBnB.info())
# print("Are there nans in column price?\n", AirBnB["price"].isnull().any())
# print("Show rows with nan:\n", AirBnB[AirBnB["price"].isnull()])
...
median = AirBnB["total_bedrooms"].median()
...
AirBnB_num = AirBnB.select_dtypes(include=[np.number]) #or: AirBnB_num = AirBnB.drop('ocean_proximity', axis=1) 
imputer.fit(AirBnB_num)
print("imputer.strategy\n", imputer.strategy)
print("imputer.statistics_\n", imputer.statistics_)
print("AirBnB_num.median\n", AirBnB_num.median().values)  # Check that this is the same as manually computing the median of each attribute:
print("AirBnB_num.mean\n", AirBnB_num.mean().values)  # Check that this is the same as manually computing the median of each attribute:
X = imputer.transform(AirBnB_num) # Transform the training set:
AirBnB_tr = pd.DataFrame(X, columns=AirBnB_num.columns,
                          index=AirBnB.index)
AirBnB_tr.loc[sample_incomplete_rows.index.values]
...
AirBnB_cat = AirBnB[['ocean_proximity']]
print("AirBnB_cat.head\n", AirBnB_cat.head(10), "\n")
try:
...
AirBnB_cat_1hot = cat_encoder.fit_transform(AirBnB_cat)
print("cat_encoder.categories_:\n", cat_encoder.categories_)
print("AirBnB_cat_1hot.toarray():\n", AirBnB_cat_1hot.toarray())
print("AirBnB_cat_1hot:\n", AirBnB_cat_1hot)
...
AirBnB_extra_attribs = attr_adder.fit_transform(AirBnB.values)

AirBnB_extra_attribs = pd.DataFrame(
    AirBnB_extra_attribs,
    columns=list(AirBnB.columns)+["rooms_per_household", "population_per_household"],
    index=AirBnB.index)
print("AirBnB_extra_attribs.head()\n", AirBnB_extra_attribs.head())
...
some_data = AirBnB.iloc[:1]
some_labels = AirBnB_labels.iloc[:1]
...
extratree_reg.fit(AirBnB_prepared, AirBnB_labels)

AirBnB_predictions = extratree_reg.predict(AirBnB_prepared)
extratree_mse = mean_squared_error(AirBnB_labels, AirBnB_predictions)
extratree_rmse = np.sqrt(extratree_mse)
print(extratree_rmse)
extratree_scores = cross_val_score(extratree_reg, AirBnB_prepared, 
                                   AirBnB_labels, 
                                   scoring = "neg_mean_squared_error", cv=10)
...
lin_grid_search = GridSearchCV(lin_reg, param_grid, cv=5,
                           scoring='neg_mean_squared_error', 
                           return_train_score=True)
lin_grid_search.fit(AirBnB_prepared, AirBnB_labels)
